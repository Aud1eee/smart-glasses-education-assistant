...
def analyze(
    ...
    heatmap_saved = False
    if plt is None:
        print(f">>> Matplotlib is unavailable in the current Windows runtime bridge: {MATPLOTLIB_ERROR}")
        print(">>> The summary statistics are still valid, but heatmap export is skipped in this runtime.")
    else:
        x = np.arange(len(df))
